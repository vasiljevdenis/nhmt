...
import { MatchImagesItem, TaskType } from "../types/tasksTypes";
...
const initialAnswers: Answer[] = TasksData.filter(el => el.answers).map(el => {
  if (el.answers) {
...
const initialScored: Scored[] = TasksData.map((el: any) => ({ slideId: el.id, scored: 0, ready: el.type === "matchImages" ? true : false, answered: false }));
      ...
      if (slideAnswers.length > 0) {
        ...
        switch (slideType) {
          ...
          case 'matchImages':
            ...
            const tasksDataItems = slideAnswers[0].items; // Данные игры

            if (tasksDataItems) {
              const fullMatch = userItems.every((item, index) => JSON.stringify(item) === JSON.stringify(tasksDataItems[index]));
              const partialMatch = userItems.some((item, index) => JSON.stringify(item) === JSON.stringify(tasksDataItems[index]));
